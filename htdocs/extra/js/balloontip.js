...
function displaytiparrow () { //function to display optional arrow image associated with tooltip
  ...
  var ieshadowwidth = (dropmenuobj.filters && dropmenuobj.filters[0])
  ...
  tiparrow.style.left = (edgeoffsetx != 0)
    ? parseInt (dropmenuobj.style.left) + dropmenuobj.offsetWidth
                                        - tiparrow.offsetWidth - 10 + "px"
  ...
  tiparrow.style.top = (edgeoffsety != 0)
    ? parseInt (dropmenuobj.style.top) + dropmenuobj.offsetHeight-tiparrow.offsetHeight
                                       - ieshadowwidth+arrowheadheight+"px"
...
function delayhidemenu () {
  delayhide = setTimeout ("dropmenuobj.style.visibility = 'hidden'; \
			   ...
			   if (enablearrowhead) tiparrow.style.visibility='hidden'",
			   disappeardelay);
}
...
function initalizetooltip () {
  ...
  for (var i = 0; i < all_divs.length; i++){
    if (reltoelement(all_divs[i])) {
      // if link has "rel" defined and it's the ID of an element on page
      all_divs [i].onmouseover = function (e) {
        var evtobj = window.event ? window.event : e
        displayballoontip (this, evtobj)
      }
      all_divs [i].onmouseout = delayhidemenu
    }
  }
}
