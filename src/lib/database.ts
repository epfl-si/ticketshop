...
export async function updateSetting(shownValue: boolean, settingId: string) {
	...
	const targetSciper = (targetUser as User).uniqueId;
	const adminSciper = user?.user?.userId;
	const isAdmin = adminSciper && targetSciper !== adminSciper;

	await log.event({
		event,
		userId: targetSciper,
		details: `${itemType} ${itemName} ${shownValue ? "enabled" : "disabled"} for ${targetSciper}`,
		metadata: {
			settingId,
			itemType,
			...
			itemName,
			targetSciper,
			...(isAdmin && adminSciper && { adminSciper }),
			status: 200,
		},
	});
...
export async function getUser(uniqueId: string): Promise<DatabaseUser | null> {
	...
	return dbUser as DatabaseUser | null;
}
